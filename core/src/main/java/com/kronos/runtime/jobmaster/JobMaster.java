...
/**
 * job master负责coordinator的实例化与处理
 * source算子会与coordinator进行交互
 * <p>
 * 1. 为何要继承runnable
 * master是在主线程上执行，还是在独立的线程池执行需要待定，暂时不处理
 *
 * @Author: jackila
...
public class JobMaster implements Runnable, JobMasterOperatorEventGateway {
    ...
    @Override
    public void sendOperatorEventToCoordinator(int task,
                                               int operatorID,
                                               OperatorEvent event) {
        try {
            operatorCoordinatorHandler.deliverOperatorEventToCoordinator(
                    task, operatorID, event);
