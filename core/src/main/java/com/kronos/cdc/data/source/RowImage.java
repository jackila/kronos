...
public class RowImage {
    ...
    public RowImage(RecordSchema recordSchema,
                    int fieldCount) {
    ...
    public void setValue(RecordField field,
                         Object value) {
        int index = field.getFieldPosition();
        setValue(index, value);
    }

    public Map<String, FieldItem> toFieldItemMap() {
        Map<String, FieldItem> valueMap = new HashMap<>();

        ObjectPath target = this.recordSchema.getTarget();
        for (RecordField field : recordSchema.getFields()) {
            valueMap.put(field.getFieldName(), new FieldItem(target, field, values[field.getFieldPosition()]));
        }

        return valueMap;
    }

    public Map<String, Object> toMap(Function<String, String> filedNameResolver,
                                     Function<Object, Object> valueResolver) {
        Map<String, Object> valueMap = new TreeMap<>();
        int i = 0;

        for (RecordField field : recordSchema.getFields()) {
            valueMap.put(filedNameResolver == null ? field.getFieldName() :
                                 filedNameResolver.apply(field.getFieldName()), valueResolver == null ? values[i] :
                                 valueResolver.apply(values[i]));
            i++;
        }
    ...
    public Pair<RecordField, Object>[] buildFieldValuePairArray(Collection<RecordField> recordFields) {
